...
This document describes the security properties of the MongrelDB Julia client
and how to report vulnerabilities.

## Overview

The MongrelDB Julia client is a Julia module that talks to `mongreldb-server`
...
- Server and network errors are mapped to the typed `MongrelDBError`
  categories (`:auth`, `:not_found`, `:constraint`, `:connection`,
  `:query`), not leaked as generic exceptions.
- NaN and Infinity are rejected with a clear error rather than corrupting
  data.

## Dependency security

The MongrelDB Julia client has no runtime dependencies beyond Julia's
standard library (the Sockets module that ships with Base). JSON is vendored
in `src/JSON.jl`. Report dependency vulnerabilities through GitHub's
...
- the MongrelDB Julia client version, Julia version, and OS,
